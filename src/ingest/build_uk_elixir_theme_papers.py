...
# -*- coding: utf-8 -*-
"""
UK + ELIXIR-theme PubMed harvester (robust + resumable)

Features
- Offline-first ROR (only call ROR API when ambiguous)
- Regex-based theme matcher (plural/hyphen tolerant, word-boundary safe)
- POST + API key for ESearch/EFetch; split failing batches on HTTP 400
- Stream-write each batch + checkpoints + clear logging
- No caps by default (set --max-per-org and --max-total to 0 = unlimited)
- Authors (name, ORCID, affiliations, UK flags, ROR IDs) serialized in authors_json
- Filter to UK presence (keep only papers with ≥1 UK author)
- Optional theme narrowing at query-time and/or require post-hoc theme match
...
DEFAULT_SLEEP_SECONDS = 0.05  # <= you asked for 0.05
DEFAULT_BATCH_SIZE = 120
DEFAULT_MAX_PER_ORG = 0  # 0 == unlimited
DEFAULT_MAX_TOTAL = 0  # 0 == unlimited
...
class RORClient:
    ...
    def query(
        self, text: str, min_score: float = 0.90, sleep_seconds: float = 0.05
    ) -> Dict[str, Any]:
...
# -------------------- PubMed helpers -----------------
def esearch_affiliation(
    org: str,
    theme_query: Optional[str],
    retmax: int,
    retstart: int,
    sleep_seconds: float,
) -> Dict[str, Any]:
...
def efetch_pmids(pmids: List[str], sleep_seconds: float) -> str:
    """
    Robust efetch: sanitize IDs, POST, split failing batches on HTTP 400, continue.
    """
...
def parse_pubmed_xml(xml_text: str) -> List[Dict[str, Any]]:
    """
    Parse PubMed XML -> list of article dicts.
    """
    if not xml_text:
        return []
    out = []
...
# -------------------- Resolver (offline-first) ---------------
def resolve_affiliation(
    aff: str,
    offline: ROROfflineMatcher,
    ror_client: Optional[RORClient],
    use_online: bool,
    min_ror_score: float,
    min_fuzzy: int,
    rules_threshold: int,
    ...
    """
    Returns: (is_uk, multi_country, ror_id, method_info)
    Strategy: offline-first. Only call ROR API when ambiguous.
    """
    if not aff:
        return False, False, None, {"method": "none", "note": "empty_aff"}
...
def main():
    ...
    ap = argparse.ArgumentParser(
        description="UK+ELIXIR-theme PubMed harvester (robust + resumable)"
    )
    ...
    pbar = tqdm(org_names, desc="Querying PubMed by org")
    for org in pbar:
        retstart = 0
        step = 100
        while True:
            js = esearch_affiliation(
                org,
                theme_query,
                retmax=step,
                retstart=retstart,
            ...
            if (
                len(ids) < step
                or (per_org_limit and retstart >= per_org_limit)
                or (max_total and len(pmids) >= max_total)
            ):
                break
        pbar.set_postfix({"pmids": f"{len(pmids):,}"})
        if max_total and len(pmids) >= max_total:
            break
    ...
    # Stage 2: fetch + parse + resolve + write
    for b in tqdm(range(batches), desc="Fetching PubMed XML"):
        ...
        xml = efetch_pmids(chunk, sleep_seconds=args.sleep_seconds)
        ...
        for aff in uniq_affs:
            aff_cache_local[aff] = resolve_affiliation(
                aff=aff,
                offline=offline,
                ror_client=ror_client,
                use_online=bool(ror_client),
                min_ror_score=args.min_ror_score,
