...
- resolver.log / progress.log             (runtime status)
- RUNNING                                 (sentinel file while job is active)

Usage
-----
No command-line arguments needed. Edit the USER CONFIG block below and run.

Dependencies
------------
pip install pandas requests rapidfuzz ujson
# optional for a live progress bar
pip install tqdm
...
PROGRESS_EVERY = 10000  # print heartbeat every N rows (when tqdm is unavailable)
USE_TQDM = True  # requires `pip install tqdm`
...
_fh = logging.FileHandler("resolver.log", mode="a", encoding="utf-8")
...
def load_ror_gb_names(ror_csv_path: str) -> Optional[pd.DataFrame]:
    if not ror_csv_path or not os.path.exists(ror_csv_path):
        progress(f"[WARN] ROR CSV not found: {ror_csv_path}")
        return None
    ...
    if not (id_col and name_col and cc_col):
        progress(
            ...
            + ", ".join(map(str, df.columns[:20].tolist()))
        )
        return None
    ...
    progress(f"[INFO] Loaded {len(out):,} GB organisation names from ROR snapshot.")
    return out
...
def main() -> int:
    # Sentinel file to confirm the script is running
    sentinel = "RUNNING"
    try:
    ...
    progress("Starting ELIXIR-UK resolver pipeline …")
    progress(
        f"Config: online={USE_ONLINE}, min_ror={MIN_ROR_SCORE}, min_fuzzy={MIN_FUZZY}, rules_thr={RULES_THRESHOLD}"
    )
    ...
    if not os.path.exists(authors_path):
        progress(f"[ERROR] Missing authors file: {authors_path}")
        return 2
    auth = pd.read_csv(authors_path)
    if "affiliation" not in auth.columns:
        progress("[ERROR] authors file must include 'affiliation' column")
        return 2
    n = len(auth)
    progress(f"Loaded authors file with {n:,} rows.")
    ...
    if os.path.exists(cache_file):
        try:
            with open(cache_file, "r") as f:
                cache = json.load(f)
            progress(f"Loaded ROR API cache with {len(cache):,} keys.")
        except Exception:
            cache = {}
    ...
    iterator = enumerate(auth.iterrows(), 1)
    if USE_TQDM and tqdm is not None:
        iterator = enumerate(tqdm(auth.iterrows(), total=n, desc="Resolving"), 1)

    for i, (_, r) in iterator:
        ...
        # Heartbeat
        if not USE_TQDM and (i % PROGRESS_EVERY == 0 or i == n):
            progress(f"Processed {i:,}/{n:,} ({i*100.0/n:.1f}%) …")
    ...
    if use_online and cache:
        try:
            ...
            progress(f"Saved ROR API cache with {len(cache):,} keys.")
        except Exception:
            pass
    ...
    progress("Wrote uk_authors_combined.csv")
    ...
    progress("Wrote affiliations_near_threshold_sample.csv")
    ...
    if os.path.exists(publications):
        ...
        if "pmid" in pubs.columns and "pmid" in res.columns:
            ...
            progress("Wrote uk_publications_clean.csv")
        else:
            progress(
                "[INFO] publications.csv present but missing 'pmid' join; skipping uk_publications_clean.csv"
            )
    else:
        progress(
            "[INFO] publications.csv not found; skipping uk_publications_clean.csv"
        )
    ...
    progress("Wrote resolve_summary.json")
